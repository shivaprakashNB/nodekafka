...
async function split(data){
   ...
   if(dataArr[1].replace(/\[|\]/g,'') == 'ATTEST1012500'){
...
function enrichItem(message, next) {
    ...
    db.mst_data.find({ deviceID:deviceID, type:"machinemaster" },function(err, retDvmap) {
        if (err) {
           return (err)
       };
        ...
        if (retDvmap) {
           ...
           console.log('piiiiiiiiiii',obj.pinno)
       }
    ...
    createItem(heartbeat, function( retResp,err) {
        if (err) {
            ...
            if (retResp) {
                ...
                //Enginehour.updateEngineHoursAndReturnDeltaV1(retResp)
...
function createItem(newObj, next) {
    console.log('jhcvsdc',newObj,newObj.deviceModel,newObj.extras.rpm)
    ...
    if(newObj.History==="H" || devicePublishTime1 <= olddata ){
        newObj.Status= "ATOLD";
    ...
    if (rpm<= 0 ) {
        checkIsEngineClosed(deviceID, pinno, function(err, retObj) {
            ...
            if (retObj && retObj.closed=='false') {
                ...
                getEngineHours(retResp.devicePublishTime, retObj.onTimestamp, function(err, retHours) {
                    if (err) {
                        console.log(err);
                    };
        ...
        checkIsEngineClosed(deviceID, pinno, function(err, retObj) {
            ...
            if (!retObj || retObj.closed=='true') {
               ...
               // enginehours.lastOnTime = new Date(enginehours.onTimestamp).valueOf() / 1000; 210:10:50'
